...

{
    ...
    private void initializeStreamingSessionIfNeeded() throws URISyntaxException {
        if (mMediaStreamingManager == null) {
            ...
            mProfile.setVideoQuality(StreamingProfile.VIDEO_QUALITY_HIGH3)
                    ...
                    .setStreamStatusConfig(new StreamingProfile.StreamStatusConfig(30))
                    .setSendTimeoutInSecond(60)
                    .setSendingBufferProfile(new StreamingProfile.SendingBufferProfile(0.2f, 0.8f, 3.0f, 20 * 1000));

            setting = new CameraStreamingSetting();
            setting.setCameraId(0)
                    .setBuiltInFaceBeautyEnabled(true)
//                    .setFrontCameraMirror(true)
//                    .setFrontCameraPreviewMirror(true)
                    .setFaceBeautySetting(new CameraStreamingSetting.FaceBeautySetting(0.9f, 0.9f, 0.9f))
    ...
    public AspectFrameLayout createViewInstance(ThemedReactContext context) {
        ...
        piliStreamPreview.setShowMode(AspectFrameLayout.SHOW_MODE.FULL);
        ...
        try {
            initializeStreamingSessionIfNeeded();
        } catch (URISyntaxException e) {
            e.printStackTrace();
        }
        Log.e("mMediaStreamingManager", "createViewInstance");

        piliStreamPreview.addOnAttachStateChangeListener(new View.OnAttachStateChangeListener() {
            @Override
            public void onViewAttachedToWindow(View v) {
                Log.e("mMediaStreamingManager", "onViewAttachedToWindow1");
                if (mMediaStreamingManager == null) {
                    return;
                }
                Log.e("mMediaStreamingManager", "onViewAttachedToWindow2");
...
//                mMediaStreamingManager.resume();
            ...
            public void onViewDetachedFromWindow(View v) {
                Log.e("mMediaStreamingManager", "onViewDetachedFromWindow");
    ...
    public void setCamera(AspectFrameLayout view, @Nullable String camera){
        Log.i("mMediaStreamingManager", "camera switched: ");
        if(camera.equals("front")){
            boolean suc = mMediaStreamingManager.switchCamera(CAMERA_FACING_ID.CAMERA_FACING_FRONT);
            Log.i("mMediaStreamingManager", "camera switched1: " + suc);
        } else if(camera.equals("back")){
            boolean suc = mMediaStreamingManager.switchCamera(CAMERA_FACING_ID.CAMERA_FACING_BACK);
            Log.i("mMediaStreamingManager", "camera switched2: " + suc);
        } else{
    ...
    public void setRtmpURL(AspectFrameLayout view, @Nullable String rtmpURL) {
        ...
        Log.e("mMediaStreamingManager", "set rtmpURL: " + rtmpURL);
    ...
    public void setZoom(AspectFrameLayout view, int zoom) {
        ...
        Log.e("mMediaStreamingManager", "setZoom: " + zoom);
        if (zoom == 0) {
            mMediaStreamingManager.pause();
            return;
        }
    ...
    public void setStarted(AspectFrameLayout view, boolean started) {
        Log.e("mMediaStreamingManager", "setStarted1: " + (mIsReady ? "mIsReady == true" : "mIsReady == false"));
        if(this.started == started){
            //ignore
            return;
        }
        Log.e("mMediaStreamingManager", "setStarted2");
...
//        mMediaStreamingManager.destroy();
    ...
    private void startStreaming() {
        mHandler.removeCallbacksAndMessages(null);
        mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_START_STREAMING), 1000);
    }
